...
public class MicroBlogPraise extends MiddlewareActionService{
	private String uid;
	...
	public void execute(){
		...
		String checkpraise = "SELECT COUNT(*) FROM wm_blog_micro_blogPraise where UID = ? AND BLOGID = ? ";
		Long checkpraisecount = middlewareService.findCount(checkpraise,uid,blogid);
		if(checkpraisecount> 0){
			responseInfo("-3","已经点赞");
			return;
		}else{
			String praisesql = "INSERT INTO wm_blog_micro_blogPraise (UID,BLOGID,TYPE,ADDIP)VALUES(?,?,?,?)";
		...
		//更新微博表的赞数量
		String sql = "UPDATE wm_blog_micro_info SET PRAISENUMBER = PRAISENUMBER+1 WHERE ID = ?";
		...
		String updateState = "UPDATE wm_user_communityInfo SET MICROPRAISECHANGENUMBER = MICROPRAISECHANGENUMBER+1 where UID = ?";
